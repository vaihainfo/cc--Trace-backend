    ...
    if (exportType === "all") {
      ...
      */
      ...
      if (isNotReel === 'true') {
        ...
        headerRow = worksheet.addRow([
          "Sr No.", "Country", "State", "Process Date", "Data Entry Date and Time", "No. of Days", "Lint Production Start Date", "Lint Production End Date", "Seed Cotton Consumed Season", "Lint process Season choosen", "Ginner Name", "Heap Number", "Gin Lot No", "Gin Press No", "REEL Lot No", "REEL Press Nos", "No of Bales", "Lint Quantity(Kgs)", "Total Seed Cotton Consumed(Kgs)", "GOT", "Total lint cotton sold(Kgs)", "Total Bales Sold", "Total lint cotton rejected(Kgs)", "Total Bales Rejected", "Total lint cotton transfered(Kgs)", "Total Bales Transfered", "Total Lint Cotton in To be Submitted Status(Kgs)", "Total Bales in To be Submitted Status(Kgs)", "Total lint cotton in stock(Kgs)", "Total Bales in stock", "Programme", "Grey Out Status"
        ]);
      } else {
        headerRow = worksheet.addRow([
          "Sr No.", "Country", "State", "Process Date", "Data Entry Date and Time", "No. of Days", "Lint Production Start Date", "Lint Production End Date", "Seed Cotton Consumed Season", "Lint process Season choosen", "Ginner Name", "Heap Number", "Gin Lot No", "Gin Press No", "REEL Lot No", "REEL Press Nos", "No of Bales", "Lint Quantity(Kgs)", "Total Seed Cotton Consumed(Kgs)", "GOT", "Total lint cotton sold(Kgs)", "Total Bales Sold", "Total lint cotton rejected(Kgs)", "Total Bales Rejected", "Total lint cotton transfered(Kgs)", "Total Bales Transfered", "Total Lint Cotton in To be Submitted Status(Kgs)", "Total Bales in To be Submitted Status(Kgs)",  "Total lint cotton in stock(Kgs)", "Total Bales in stock", "Programme", "Village", "Grey Out Status"
        ]);
      }
      headerRow.font = { bold: true };
      ...
      for await (const [index, item] of rows.entries()) {
        ...
        totals.total_lint_qty_to_be_submitted += item.lint_qty_to_be_submitted ? Number(item.lint_qty_to_be_submitted) : 0;
        ...
        totals.total_bales_to_be_submitted += item.bales_to_be_submitted ? Number(item.bales_to_be_submitted) : 0;
...
const fetchSpinnerProcessGreyOutReport = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await SpinProcess.findAndCountAll({
      ...
      offset: offset,
      limit: limit,
      order: [[Sequelize.literal('TRIM("spinner"."name")'), 'ASC']]
    });
    return res.sendPaginationSuccess(res, rows, count);
  } catch (error: any) {
    console.log(error)
...
const fetchSpinnerGreyOutReport = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await GinSales.findAndCountAll({
      ...
      offset: offset,
      limit: limit,
      order: [[Sequelize.literal('TRIM("buyerdata"."name")'), 'ASC']]
    });
    return res.sendPaginationSuccess(res, rows, count);
  } catch (error: any) {
    console.log(error)
    ...
    const { count, rows }: any = await SpinProcessYarnSelection.findAndCountAll(
      {
        attributes: [
          ...
          [Sequelize.col('"sales"."knitter_id"'), "knitter_id"],
          [Sequelize.col('"sales"."knitter"."name'), "knitter"],
          [Sequelize.col('"sales"."knitter"."country"."county_name"'), "country"],
          [Sequelize.col('"sales"."knitter"."state"."state_name"'), "state"],
          [Sequelize.col('"sales"."weaver"."name'), "weaver"],
          [Sequelize.literal('"sales"."total_qty"'), "total_qty"],
          [Sequelize.literal('"sales"."invoice_no"'), "invoice_no"],
          [Sequelize.literal('"sales"."batch_lot_no"'), "batch_lot_no"],
        ...
        group: [
          ...
          "sales.knitter.id",
          "sales.program.id",
          "sales.knitter.country.id",
          "sales.knitter.state.id",
        ],
        order: [["knitter", "asc"]],
        offset: offset,
        limit: limit,
...
const fetchKnitterYarnProcess = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await KnitProcess.findAndCountAll({
      where: whereCondition,
      include: include,
      order: [[Sequelize.literal('TRIM("knitter"."name")'), "ASC"]],
    });
...
const exportKnitterYarnProcess = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await KnitProcess.findAndCountAll({
      where: whereCondition,
      include: include,
      order: [[Sequelize.literal('LOWER(TRIM("knitter"."name"))'), "ASC"]],
    });
...
const exportKnitterSale = async (req: Request, res: Response) => {
  ...
  try {
    ...
    for await (const [index, item] of rows.entries()) {
      ...
      totals.fabric_weight += Number(rowValues.fabric_weight);
      totals.total_fabric_weight += Number(rowValues.total_fabric_weight);
    ...
    const { count, rows }: any = await SpinProcessYarnSelection.findAndCountAll(
      {
        attributes: [
          ...
          [Sequelize.col('"sales"."knitter"."name"'), "knitter"],
          [Sequelize.col('"sales"."weaver"."name'), "weaver"],
          [Sequelize.col('"sales"."weaver"."country"."county_name"'), "country"],
          [Sequelize.col('"sales"."weaver"."state"."state_name"'), "state"],
          [Sequelize.literal('"sales"."total_qty"'), "total_qty"],
          [Sequelize.literal('"sales"."invoice_no"'), "invoice_no"],
          [Sequelize.literal('"sales"."batch_lot_no"'), "batch_lot_no"],
          // [Sequelize.literal('"process"."reel_lot_no"'), "reel_lot_no"],
        ...
        group: [
          ...
          "sales.knitter.id",
          "sales.program.id",
          "sales.weaver.country.id",
          "sales.weaver.state.id",
        ],
        order: [["weaver", "ASC"]],
        offset: offset,
        limit: limit,
...
const fetchWeaverYarnProcess = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await WeaverProcess.findAndCountAll({
      where: whereCondition,
      include: include,
      order: [[Sequelize.literal('TRIM("weaver"."name")'), "ASC"]],
    });
...
const exportWeaverYarnProcess = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await WeaverProcess.findAndCountAll({
      where: whereCondition,
      include: include,
      order: [[Sequelize.literal('LOWER(TRIM("weaver"."name"))'), "ASC"]],
    });
    ...
    for await (const [index, item] of result.entries()) {
      ...
      totals.total_yarn += Number(rowValues.total_yarn);
      totals.netLength += Number(rowValues.netLength);
...
const exportWeaverSale = async (req: Request, res: Response) => {
  ...
  try {
    ...
    for await (const [index, item] of rows.entries()) {
      ...
      totals.fabric_length += Number(rowValues.fabric_length);
      totals.total_fabric_length += Number(rowValues.total_fabric_length);
    ...
    let rows: any = await Promise.all([
      WeaverFabricSelection.findAll({
        attributes: [
          ...
          [Sequelize.col('"sales"."fabric_id"'), "fabric_id"],
          [Sequelize.col('"sales"."buyer"."name"'), "garment"],
          [Sequelize.col('"sales"."buyer"."country"."county_name"'), "country_name"],
          [Sequelize.col('"sales"."buyer"."state"."state_name"'), "state_name"],
...
const exportGarmentSales = async (req: Request, res: Response) => {
  ...
  try {
    ...
    for await (const [index, item] of result.entries()) {
      ...
      totals.no_of_boxes += Number(rowValues.no_of_boxes);
      totals.no_of_pieces += Number(rowValues.no_of_pieces);
...
const fetchGinnerLintStockPagination = async (req: Request, res: Response) => {
  ...
  const limit = Number(req.query.limit) || 10;
  const offset = (page - 1) * limit;
  const { ginnerId, seasonId, programId, brandId, countryId, stateId, districtId }: any = req.query;
  const whereCondition: any = {};
  const transactionWhere: any = {};
  const ginBaleWhere: any = {};
  const baleSelectionWhere: any = {};
  ...
  try {
    ...
    if (districtId) {
      const idArray: number[] = districtId
        .split(",")
        .map((id: any) => parseInt(id, 10));
      whereCondition["$ginner.district_id$"] = { [Op.in]: idArray };
    }
...
const exportGinnerLintStockReport = async (req: Request, res: Response) => {
  const excelFilePath = path.join("./upload", "excel-ginner-lint-cotton-stock-report.xlsx");

  const searchTerm = req.query.search || "";
  const page = Number(req.query.page) || 1;
  const limit = Number(req.query.limit) || 10;
  const offset = (page - 1) * limit;
  const { exportType, ginnerId, seasonId, programId, brandId, countryId, stateId, districtId }: any = req.query;
  const whereCondition: any = {};
  const transactionWhere: any = {};
  const ginBaleWhere: any = {};
  const baleSelectionWhere: any = {};
  ...
  try {
    ...
    if (exportType === "all") {
      return res.status(200).send({
        success: true,
        messgage: "File successfully Generated",
        data: process.env.BASE_URL + "ginner-lint-stock.xlsx",
      });
      ...
      res.status(200).send({
        success: true,
        messgage: "File successfully Generated",
        data: process.env.BASE_URL + "excel-ginner-lint-cotton-stock-report.xlsx",
      });
    }
  } catch (error: any) {
    console.error("Error appending data:", error);
...
const fetchSpinnerYarnOrdersPagination = async (req: Request, res: Response) => {
  const searchTerm = req.query.search || "";
  const page = Number(req.query.page) || 1;
  const limit = Number(req.query.limit) || 10;
  const { spinnerId, seasonId, programId, brandId, countryId, startDate, endDate }: any = req.query;
  ...
  try {
    ...
    if (searchTerm) {
      whereCondition[Op.or] = [
        { "$spinner.name$": { [Op.iLike]: `%${searchTerm}%` } },
        { "$season.name$": { [Op.iLike]: `%${searchTerm}%` } },
        { fabricMillOrderRefNumber: { [Op.iLike]: `%${searchTerm}%` } },
        { spinnerInternalOrderNumber: { [Op.iLike]: `%${searchTerm}%` } },
        { reel_yarn_order_number: { [Op.iLike]: `%${searchTerm}%` } },
      ];
    }
    if (spinnerId) {
      const idArray: number[] = spinnerId
        .split(",")
        .map((id: any) => parseInt(id, 10));
      whereCondition.spinnerId = { [Op.in]: idArray };
    }
    if (brandId) {
      const idArray: number[] = brandId
        .split(",")
    ...
    const { count, rows } = await SpinnerYarnOrder.findAndCountAll({
      distinct: true, 
    ...
    // Combine the data
    const enrichedYarnOrders = rows.map((order: any) => {
    ...
    return res.sendPaginationSuccess(res, enrichedYarnOrders, count);
...
const exportSpinnerYarnOrders = async (req: Request, res: Response) => {

  const excelFilePath = path.join("./upload", "excel-spinner-yarn-order.xlsx");

  const searchTerm = req.query.search || "";
  const page = Number(req.query.page) || 1;
  const limit = Number(req.query.limit) || 10;
  const { exportType,spinnerId, seasonId, brandId, countryId, startDate, endDate }: any = req.query;
  const offset = (page - 1) * limit;
  const whereCondition: any = {};
  console.log('reqlog',req);
  try {
    if (exportType === "all") {
      return res.status(200).send({
        success: true,
        messgage: "File successfully Generated",
        data: process.env.BASE_URL + "spinner-yarn-order.xlsx",
      });
    } 
    else{
      if (searchTerm) {
    ...
    // Get base yarn orders
    const  rows = await SpinnerYarnOrder.findAll({
      distinct: true, 
      order: [["createdAt", "DESC"]],
      include: [
        {
      ...
      for await (const [index, item] of rows.entries()) {
       ...
       let rowValues;
        ...
        res.status(200).send({
          success: true,
          messgage: "File successfully Generated",
          data: process.env.BASE_URL + "excel-spinner-yarn-order.xlsx",
        });
     }
  }catch (error: any) {
    console.log(error);
...
export {
  ...
  fetchSpinnerYarnOrdersPagination,
  exportSpinnerYarnOrders
};
