    ...
    if (exportType === "all") {
      ...
      if (isOrganic === 'true') {
        headerRow = worksheet.addRow([
          "Sr No.", "Process Date", "Data Entry Date and Time", "Lint process Season choosen", "Ginner Name", "Heap Number", "Gin Lot No", "Gin Press No",  "No of Bales", "Lint Quantity(Kgs)", "Programme", "Grey Out Status"
        ]);
      }
      else if (isBrand === 'true') {
        headerRow = worksheet.addRow([
    ...
    if (exportType === "all") {
      ...
      const gin = await GinProcess.findAll({
        ...
        attributes: [
          ...
          [Sequelize.literal('lot_no'), 'lot_no'],
  ...
  try {
    if (exportType === "all") {
      ...
      let rows = await Ginner.findAll({
        where: whereCondition, attributes: ["id", "name", "address"],
        offset: offset,
        limit: limit,
      });
...
const fetchPscpPrecurement = async (req: Request, res: Response) => {
  try {
    ...
    for await (const [index, item] of result.entries()) {
      ...
      obj.balace_stock =
        obj.no_of_bales > obj.sold_bales ? Number(obj.no_of_bales - obj.sold_bales) : 0;
