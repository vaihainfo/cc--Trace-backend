    ...
    try {
        ...
        if (exportType === "all") {
            ...
            for await (const [index, item] of transactions.entries()) {
                ...
                if (isBrand === 'true') {
                 rowValues = {
                    ...
                    qty_purchased: item.qty_purchased ? Number(item.qty_purchased).toFixed(1) : 0,
                    ...
                    available_cotton: item.available_cotton ? Number(item.available_cotton).toFixed(1) : 0,
                    rate: item.rate ? Number(item.rate).toFixed(1) : 0,
            ...
            const rowValues = {
              ...
              village:"",
              transactionId:"Total",
              qty_purchased:totals.qty_purchased.toFixed(1),
              available_cotton:totals.available_cotton.toFixed(1),
              rate:totals.rate.toFixed(1),
              program:"",
              vehicle: "",
              payment_method: "",
              ginner: "",
