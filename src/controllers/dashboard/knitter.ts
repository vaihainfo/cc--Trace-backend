...
const getQueryParams = async (
  ...
  try {
    ...
    await validator.validate(knitter);
...
const getYarnCompareCount = async (
  ...
  try {
    ...
    const knitterWhere = getKnitterSalesWhereQuery(reqData);
...
const getYarnProcessedData = async (
  ...
  const result = await KnitProcess.findAll({
    attributes: [
...
const getFabricProcessedData = async (
  ...
  const result = await KnitProcess.findAll({
    attributes: [
      [Sequelize.fn('SUM', Sequelize.col('total_fabric_weight')), 'processed'],
      [Sequelize.col('season.name'), 'seasonName'],
      [Sequelize.col('season.id'), 'seasonId']
    ],
    include: [{
...
const getTopYarnProcuredData = async (
  ...
  const result = await SpinSales.findAll({
    attributes: [
      ...
      [Sequelize.col(`spinner.name`), 'buyerName'],
    ],
    include: [{
      model: Knitter,
      as: 'knitter',
