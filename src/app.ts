  ...
  const dbConn = require('./util/dbConn'); 
  const sequelize1 = dbConn.default; 
  ...
  app.get('/update-greyout', async (req, res) => {
    try {
        const excelFilePath = path.resolve(__dirname, '..', 'Leaspin Process Report - Need to be greyout.xlsx');
        const workbook = xlsx.readFile(excelFilePath);
        const sheetName = workbook.SheetNames[0];
        let dataArr = xlsx.utils.sheet_to_json(workbook.Sheets[sheetName], { raw: false, defval: '' });

        // Make sure the database is connected before running the update
        await connectToDb();

        const messages = await updateDatabase(dataArr); 
...
const updateDatabase = async (dataArr: Array<Record<string, any>>): Promise<string[]> => {
  const messages: string[] = []; // Array to accumulate messages
      ...
      for (let i = 1; i < dataArr.length; i++) { 
          ...
          if (row['__EMPTY_5']) { 
              ...
              if (ginnerResults.length === 1) {
                  ...
                  messages.push(`\nUpdated greyout_status for id: ${ginner.id}`); // Add message to array
              } else if (ginnerResults.length > 1) {
                  messages.push(`More than one record found; not updating for reel_lot_no: ${row['__EMPTY_5']}`);
              } else {
                  messages.push(`No records found to update for reel_lot_no: ${row['__EMPTY_5']}`);
              }
          }
      }
  } catch (error:any) {
      console.error("Error during database update:", error);
      messages.push("Error during database update: " + error.message); // Log error message
  }
